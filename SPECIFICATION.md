# Tenex Finance Specification

Tenex  is a partial rewrite and redesign of the Solidly architecture.

## Definitions

- TENEX: The native token in the Tenex ecosystem. It is emitted by the Minter and is an ERC-20 compliant token.
...
- Pool: AMM constant-product implementation similar to Uniswap  liquidity pools.

## Protocol Upgradability

Tenex  is immutable. To allow improving the protocol, 
...
AMM constant-product implementation similar to Uniswap  liquidity pools. 
...
's constant product formula. The invariant `K` can temporarily decrease
...
Standard Uniswap-like Router interface. Supports multi-pool swaps, lp deposits 
...
The VotingEscrow contracts allow users to escrow their TENEX tokens in an veTENEX NFT. 
...
of four years. veTENEX NFT vote weights can be used to vote for pools, which in turn
...
There are three states that veTENEX NFTs can be in: `NORMAL`, `LOCKED`, `MANAGED`.
...
amount of veTENEX that was locked to create it. The NFT's voting power will also
...
- Can create a NFT by escrowing TENEX tokens and "locking" them for a time period.
...
- Can withdraw escrowed TENEX tokens once the NFT lock expires (requires normal NFT). 
- Can add to an existing NFT position by escrowing additional TENEX tokens (requires normal or normal permanent or managed NFT).
...
- Can delegate votes to other `tokenId`s for use in Tenex governance to other addresses based on voting power (requires normal permanent or managed NFT). Voting power retrieved from `getVotes` and `getPastVotes` does not reveal locked amount balances and are used only for voting. 

See `VOTINGESCROW.md` for a visual respresentation.

In addition, Tenex supports "managed NFTs" (also known as an "(m)veNFT") which aggregates NFT voting power whilst perpetually locking the underlying tokens. These NFTs function as a single NFT, with rewards accrued by the NFT going to the manager, who can then distribute (net of fees) to the depositors. These NFTs are permanently locked by default.
...
- For (m)veNFT implementations, refer to the official [repository](https://github.com/Tenex-finance/AutoCompounder).

### Minter

The minting contract handles emissions for the Tenex protocol. Emissions
...
are calculated based on the locked and unlocked TENEX one second prior to epoch flip.
veNFTs will earn rebases proportionally based on their contribution to the total locked
TENEX. Rebase claims against expired veNFTs will be distributed as unlocked TENEX to the 
owner of the veNFT. 
...
as well as gauge creation in the Tenex ecosystem. Votes can be cast once 
...
- Can create gauge and reward contracts for a pool (must be a pool created as a part of Tenex).
...
The fee voting reward derives from the fees `(the fees which is accumulated by swaps or any trade in that Pool)` relinquished by LP depositors 
...
Locked rewards are TENEX token rewards that have been compounded into the managed NFT (usually rebases but can also include non-TENEX rewards that have converted to TENEX to be compounded into the NFT). This contract functions similar to `PoolFees`, as it separates "reward" TENEX from the "locked" VotingEscrow TENEX. These rewards are not distributed and are returned to `VotingEscrow` when a user withdraws their NFT from a managed NFT. 
