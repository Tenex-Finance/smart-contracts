...
abstract contract Reward is IReward, ERC2771Context, ReentrancyGuard {
    ...
    function _writeCheckpoint(uint256 tokenId, uint256 balance) internal {
        ...
        if (
            _nCheckPoints > 0 &&
            TenexTimeLibrary.epochStart(checkpoints[tokenId][_nCheckPoints - 1].timestamp) ==
            TenexTimeLibrary.epochStart(_timestamp)
        ) {
    ...
    function _writeSupplyCheckpoint() internal {
        ...
        if (
            _nCheckPoints > 0 &&
            TenexTimeLibrary.epochStart(supplyCheckpoints[_nCheckPoints - 1].timestamp) ==
            TenexTimeLibrary.epochStart(_timestamp)
        ) {
    ...
    function earned(address token, uint256 tokenId) public view returns (uint256) {
        ...
        uint256 reward = 0;
        uint256 _supply = 1;
        uint256 _currTs = TenexTimeLibrary.epochStart(lastEarn[token][tokenId]); // take epoch last claimed in as starting point
        uint256 _index = getPriorBalanceIndex(tokenId, _currTs);
        Checkpoint memory cp0 = checkpoints[tokenId][_index];

        // accounts for case where lastEarn is before first checkpoint
        _currTs = Math.max(_currTs, TenexTimeLibrary.epochStart(cp0.timestamp));

        // get epochs between current epoch and first checkpoint in same epoch as last claim
        uint256 numEpochs = (TenexTimeLibrary.epochStart(block.timestamp) - _currTs) / DURATION;
    ...
    function _notifyRewardAmount(address sender, address token, uint256 amount) internal {
        ...
        uint256 epochStart = TenexTimeLibrary.epochStart(block.timestamp);
