...
contract MinterTest is BaseTest {
    ...
    function testTailEmissionFlipsWhenWeeklyEmissionDecaysBelowTailStart() public {
        ...
        skipToNextEpoch(1);
        minter.updatePeriod();
        assertApproxEqRel(TENEX.balanceOf(address(voter)), 6_010_270 * 1e18, 1e12);
        ...
        minter.updatePeriod();
        assertApproxEqAbs(TENEX.balanceOf(address(voter)), 168_932 * 1e18, TOKEN_1);
    ...
    function testMinterWeeklyDistribute() public {
        ...
        uint256 pre = TENEX.balanceOf(address(voter));
        skipToNextEpoch(1);
        minter.updatePeriod();
        assertEq(distributor.claimable(tokenId), 57);
        // emissions decay by 1% after one epoch
        uint256 post = TENEX.balanceOf(address(voter));
        ...
        minter.updatePeriod();
        post = TENEX.balanceOf(address(voter));
