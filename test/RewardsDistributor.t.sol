...
contract RewardsDistributorTest is BaseTest {
    ...
    function testInitialize() public {
        ...
        assertEq(distributor.token(), address(TENEX));
    ...
    function testClaimWithExpiredNFT() public {
        // test reward claims to expired NFTs are distributed as unlocked TENEX
        // ts: 608402
        TENEX.approve(address(escrow), TOKEN_1M);
        ...
        assertEq(post - pre, rebase); // expired rebase distributed as unlocked TENEX
    ...
    function testClaimRebaseWithManagedLocks() public {
        ...
        assertLt(TENEX.balanceOf(address(distributor)), 100); // dust
    }

    function testClaimRebaseWithDepositManaged() public {
        minter.updatePeriod(); // does nothing
        vm.startPrank(address(owner2));
        TENEX.approve(address(escrow), TOKEN_10M);
    ...
    function testClaimOnLockedEnd() public {
        ...
        // Rebase should deposit into veNFT one second before expiry
        uint256 balanceBefore = TENEX.balanceOf(address(owner));
        distributor.claim(tokenId);
        assertGt(TENEX.balanceOf(address(owner)), balanceBefore);
    }
}
