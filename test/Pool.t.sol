...
contract PoolTest is BaseTest {
    ...
    function increaseLock() public {
        ...
        assertEq(TENEX.balanceOf(address(escrow)), TOKEN_1);
    }
    ...
    function votingEscrowMerge() public {
        ...
        assertEq(TENEX.balanceOf(address(escrow)), 2 * TOKEN_1);
        ...
        assertEq(TENEX.balanceOf(address(escrow)), 3 * TOKEN_1);
    ...
    function deployPoolFactoryGauge() public {
        ...
        TENEX.approve(address(gaugeFactory), 15 * TOKEN_100K);
    ...
    function addGaugeAndVotingRewards() public {
        ...
        TENEX.approve(address(bribeVotingReward), POOL_1);

        bribeVotingReward.notifyRewardAmount(address(TENEX), POOL_1);
    ...
    function createLock2() public {
        ...
        skip(1);
        assertGt(escrow.balanceOfNFT(1), 995063075414519385);
        assertEq(TENEX.balanceOf(address(escrow)), 4 * TOKEN_1);
    }
    ...
    function gaugeDistributeBasedOnVoting() public {
        ...
        deal(address(TENEX), address(minter), POOL_1);

        vm.startPrank(address(minter));
        TENEX.approve(address(voter), POOL_1);
    ...
    function gaugeClaimRewards() public {
        ...
        gauge.deposit(POOL_1);
        skip(1);
        uint256 before = TENEX.balanceOf(address(owner));
        skip(1);
        uint256 earned = gauge.earned(address(owner));
        gauge.getReward(address(owner));
        skip(1);
        uint256 after_ = TENEX.balanceOf(address(owner));
    ...
    function gaugeClaimRewardsOwner3NextCycle() public {
        ...
        owner3.approve(address(pool), address(gauge), POOL_1);
        owner3.deposit(address(gauge), POOL_1);
        uint256 before = TENEX.balanceOf(address(owner3));
        skip(1);
        owner3.getGaugeReward(address(gauge), address(owner3));
        uint256 after_ = TENEX.balanceOf(address(owner3));
        uint256 received = after_ - before;
        assertGt(received, 0);
