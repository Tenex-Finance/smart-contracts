...
contract VotingEscrowTest is BaseTest {
    ...
    function testDepositForWithManagedNFT() public {
        ...
        assertEq(TENEX.allowance(address(escrow), address(lockedManagedReward)), 0);
        ...
        uint256 pre = TENEX.balanceOf(address(lockedManagedReward));
        ...
        emit NotifyReward(address(escrow), address(TENEX), 604800, reward);
        vm.expectEmit(false, false, false, true, address(escrow));
        emit MetadataUpdate(mTokenId);
        escrow.depositFor(mTokenId, reward);
        uint256 post = TENEX.balanceOf(address(lockedManagedReward));
    ...
    function testCreateLock() public {
        TENEX.approve(address(escrow), 1e25);
    ...
    function testWithdraw() public {
        ...
        uint256 preBalance = TENEX.balanceOf(address(owner));

        skipAndRoll(lockDuration);
        escrow.withdraw(1);

        uint256 postBalance = TENEX.balanceOf(address(owner));
    ...
    function testMergeWithFromLockTimeGreaterThanToLockTime() public {
        ...
        uint256 tenexSupply = escrow.supply();
        ...
        assertEq(escrow.supply(), tenexSupply);
    ...
    function testCannotSplitOverflow() public {
        ...
        uint256 escrowBalance = TENEX.balanceOf(address(escrow));
    ...
    function testCannotDelegateBySigWithInvalidNonce() public {
        ...
        deal(address(TENEX), alice, TOKEN_100K);
    ...
    /// current blast timestamp >= 1600000000
