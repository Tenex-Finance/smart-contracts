...
contract ManagedNftTest is BaseTest {
    ...
    function testWithdrawManagedWithLockedReward() public {
        ...
        freeManagedReward = FreeManagedReward(escrow.managedToFree(mTokenId));
        assertEq(TENEX.balanceOf(address(freeManagedReward)), 0);
        ...
        assertEq(TENEX.balanceOf(address(escrow)), TOKEN_1 * 2);
        assertEq(TENEX.balanceOf(address(lockedManagedReward)), 0);
    }
    ...
    function testWithdrawManagedWithFreeReward() public {
        ...
        TENEX.approve(address(freeManagedReward), TOKEN_1);
        freeManagedReward.notifyRewardAmount(address(TENEX), TOKEN_1);

        assertEq(escrow.supply(), supply);
        assertEq(TENEX.balanceOf(address(freeManagedReward)), TOKEN_1);
        assertEq(freeManagedReward.tokenRewardsPerEpoch(address(TENEX), 604800), TOKEN_1);
        ...
        uint256 pre = TENEX.balanceOf(address(owner));
        freeManagedReward.getReward(tokenId, rewards);
        uint256 post = TENEX.balanceOf(address(owner));
    ...
    function testWithdrawManagedWithLockedRewardWithDelegatingManagedNFT() public {
        ...
        assertEq(TENEX.balanceOf(address(escrow)), TOKEN_1 * 3);
        assertEq(TENEX.balanceOf(address(lockedManagedReward)), 0);
    }
