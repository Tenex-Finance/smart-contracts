...
contract MinterTestFlow is ExtendedBaseTest {
    ...
    function testMinterRebaseFlow() public {
        /// epoch 0
        minter.updatePeriod();
        assertEq(TENEX.balanceOf(address(voter)), 0);

        TENEX.approve(address(escrow), TOKEN_100K);
        ...
        minter.updatePeriod();
        assertEq(TENEX.balanceOf(address(voter)), expectedMint);

        address[] memory gauges = new address[](1);
        gauges[0] = address(gauge);

        uint256 epochStart = _getEpochStart(block.timestamp);
        assertEq(TENEX.allowance(address(voter), address(gauge)), 0);
        voter.distribute(gauges);
        assertEq(TENEX.allowance(address(voter), address(gauge)), 0);
        assertApproxEqRel(TENEX.balanceOf(address(gauge)), expectedMint / 2, 1e6);
        assertApproxEqRel(TENEX.balanceOf(address(voter)), expectedMint / 2, 1e6);
        ...
        voter.distribute(gauges);
        assertApproxEqRel(TENEX.balanceOf(address(gauge2)), expectedMint / 2, 1e6);
        assertLt(TENEX.balanceOf(address(voter)), 1e6); // dust
        skipAndRoll(1);
        ...
        /// epoch 2
        skipToNextEpoch(1);
        uint256 balance = TENEX.balanceOf(address(gauge));
        ...
        assertLt(TENEX.balanceOf(address(voter)), 1e6);
        assertApproxEqRel(TENEX.balanceOf(address(gauge)), balance, 1e6);
        assertApproxEqRel(TENEX.balanceOf(address(gauge)), balance, 1e6);
        ...
        minter.updatePeriod();
        /// total TENEX supply ~1005457263, tail emissions ~= .29% of total supply
        /// 1005457263 ~= 50_000_000 initial supply + emissions until now
        assertApproxEqAbs(TENEX.balanceOf(address(voter)), 3_000_697 * TOKEN_1, TOKEN_1);
        voter.distribute(0, voter.length());
        ...
        minter.updatePeriod();
        /// total TENEX supply ~1008887347, tail emissions .30% of total supply
        assertApproxEqAbs(TENEX.balanceOf(address(voter)), 3_110_487 * TOKEN_1, TOKEN_1);
        voter.distribute(0, voter.length());
        ...
        minter.updatePeriod();
        /// total TENEX supply ~1012328065, tail emissions .30% of total supply
        assertApproxEqAbs(TENEX.balanceOf(address(voter)), 3_120_612 * TOKEN_1, TOKEN_1);
        voter.distribute(0, voter.length());
        ...
        minter.updatePeriod();
        /// total TENEX supply ~1015678216, tail emissions .29% of total supply
        assertApproxEqAbs(TENEX.balanceOf(address(voter)), 3_029_777 * TOKEN_1, TOKEN_1);
        voter.distribute(0, voter.length());
    }
